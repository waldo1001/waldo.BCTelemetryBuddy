...
### Added
- **`mcpEnvBuilder.ts`**: Extracted MCP environment variable construction into a dedicated, testable module with 10 unit tests covering all env var combinations.
...
### Fixed
- **Build Errors**: Fixed TypeScript compilation errors that prevented CI builds from succeeding
  - Fixed broken template literal syntax in chatParticipant.ts system prompt
  - Fixed incorrect property reference `appInsightsAppId` → `applicationInsightsAppId` in extension.ts configuration validation
...
### Changed
- **MCP Dependency**: Updated bundled MCP to v2.3.3 with improved config loading and test reliability

### Fixed
- **Agent Installation**: Fixed incomplete variable renaming in `installAgentsCommand` function (chatmodeDir → agentDir, existingChatmodes → existingAgents)
- **CI Build**: Resolved TypeScript compilation errors that prevented v1.3.2 from building in CI
...
### Changed
- **Documentation**: Updated README privacy section to clarify installation ID storage
  - Installation IDs stored in VS Code user profile (not workspace)
  - Matches MCP v2.2.5 behavior for consistent privacy approach
...
### Fixed
- **MCP Bundling**: Updated bundled MCP to v2.2.4 with critical config loading fix
  - Fixes regression from v2.2.1 where config files stopped loading from workspace root
  - `workspacePath` now correctly falls back to `BCTB_WORKSPACE_PATH` environment variable
  - `${workspaceFolder}` placeholder in config files now properly expands
  - Resolves "workspacePath is required" errors when using extension with valid config file
