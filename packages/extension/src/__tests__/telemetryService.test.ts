...
describe('TelemetryService', () => {
    ...
    describe('Initialization', () => {
        ...
        it('should not be configured when tenantId is missing', () => {
            const newMockConfig = {
                get: jest.fn((key: string, defaultValue?: any) => {
                    if (key === 'mcp.tenantId') return '';
                    const config: Record<string, any> = {
                        'mcp.connectionName': 'test-connection',
                        'mcp.clientId': 'test-client-id',
                        'mcp.clientSecret': '',
    ...
    describe('executeKQL()', () => {
        it('should execute KQL query successfully', async () => {
            ...
            expect(mockKustoService.executeQuery).toHaveBeenCalledWith(kql);
        ...
        it('should use cache when available', async () => {
            ...
            expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(expect.stringContaining('cache hit'));
        });
