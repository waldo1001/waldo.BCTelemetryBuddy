...
export interface KBContributeResult {
    ...
    articleBody?: string;
    /** Raw article markdown (frontmatter + content). Display this in a ~~~markdown fence so users can select-all and copy without nested-fence issues. Only present when success: false / no token. */
...
export class KnowledgeBaseService {
    ...
    /**
     * Contribute an article to the community KB by creating a GitHub Issue.
     *
     * Without a token: returns a pre-filled issues/new URL and the full article
     * body so the user can submit manually (success: false, no API calls made).
     *
     * With a token (knowledgeBase.githubToken or BCTB_GITHUB_TOKEN env var):
     * creates the issue via API and returns the issue URL (success: true).
     * Required scope: issues:write (public_repo is sufficient for public repos).
     */
    async contributeArticle(params: KBSaveParams): Promise<KBContributeResult> {
        ...
        const tagsList = (params.tags && params.tags.length > 0) ? params.tags.join(', ') : '(none)';
        const eventIdsList = (params.eventIds && params.eventIds.length > 0) ? params.eventIds.join(', ') : '(none)';
        const issueBodyLines: string[] = [
            '## Community Knowledge Base Contribution',
            '',
            `**Category:** ${params.category}`,
            `**Tags:** ${tagsList}`,
            `**Event IDs:** ${eventIdsList}`,
        ];
        if (params.appliesTo) issueBodyLines.push(`**Applies to:** ${params.appliesTo}`);
        issueBodyLines.push(
            '',
            `Please add this file to: \`${filePath}\``,
            '',
            '### Article Content',
            '',
            '```markdown',
            articleContent.trimEnd(),
            '```',
            '',
        ...
        if (!token) {
            const encodedTitle = encodeURIComponent(`KB: ${params.title}`);
            const preFilledUrl = `https://github.com/${repoInfo.owner}/${repoInfo.repo}/issues/new?title=${encodedTitle}&labels=kb-contribution`;
            return {
                success: false,
                id: slug,
                issueUrl: preFilledUrl,
                articleBody: issueBody,
                articleMarkdown: articleContent.trimEnd(),
                message: `No GitHub token configured. To submit this community contribution:\n\n1. Open this URL to create a GitHub issue:\n   ${preFilledUrl}\n\n2. Paste the content in \`articleMarkdown\` below into the issue description.\n\nTo skip this manual step in future, set \`BCTB_GITHUB_TOKEN\` in your environment or configure \`knowledgeBase.githubToken\` in .bctb-config.json.`,
            };
        }

        const issueResp = await this.client.post(
        ...
        return {
            ...
            id: slug,
            issueUrl,
            message: `Issue created: ${issueUrl}. A maintainer will review and add it to the community KB.`,
        };
    }
