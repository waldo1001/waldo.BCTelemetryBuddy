...
import { generateRunReport, appendToDailyReport } from '../../agent/report';
...
// ─── appendToDailyReport Tests ───────────────────────────────────────────────

describe('appendToDailyReport', () => {
    let tmpDir: string;
    ...
    it('creates a new daily doc with header on first run', () => {
        const relPath = appendToDailyReport(tmpDir, 'error-monitor', runLog, '### Run #3 — 14:30 UTC\n\nAll clear.');

        expect(relPath).toBe('docs/2026-03-02-error-monitor.md');
        const content = fs.readFileSync(path.join(tmpDir, 'docs', '2026-03-02-error-monitor.md'), 'utf-8');
        expect(content).toContain('# error-monitor — Investigation Report — 2026-03-02');
    ...
    it('appends to existing daily doc on subsequent runs', () => {
        appendToDailyReport(tmpDir, 'error-monitor', runLog, '### Run #3\n\nFirst run.');

        const runLog2 = makeRunLog({ agentName: 'error-monitor', timestamp: '2026-03-02T15:30:00.000Z', runId: 4 });
        appendToDailyReport(tmpDir, 'error-monitor', runLog2, '### Run #4\n\nSecond run.');

        const content = fs.readFileSync(path.join(tmpDir, 'docs', '2026-03-02-error-monitor.md'), 'utf-8');
        // Header should appear only once
        const headerCount = (content.match(/# error-monitor — Investigation Report/g) || []).length;
        expect(headerCount).toBe(1);
        // Both run sections should be present
        expect(content).toContain('### Run #3');
        expect(content).toContain('First run.');
        expect(content).toContain('### Run #4');
        expect(content).toContain('Second run.');
    });

    it('links to previous day report when it exists', () => {
        // Create yesterday's doc
        const docsDir = path.join(tmpDir, 'docs');
        fs.mkdirSync(docsDir, { recursive: true });
        fs.writeFileSync(path.join(docsDir, '2026-03-01-error-monitor.md'), '# Yesterday', 'utf-8');

        appendToDailyReport(tmpDir, 'error-monitor', runLog, '### Run #3\n\nToday.');

        const content = fs.readFileSync(path.join(docsDir, '2026-03-02-error-monitor.md'), 'utf-8');
        expect(content).toContain('Previous report: [2026-03-01](2026-03-01-error-monitor.md)');
    });

    it('does not link to previous day when it does not exist', () => {
        appendToDailyReport(tmpDir, 'error-monitor', runLog, '### Run #3\n\nFirst ever.');

        const content = fs.readFileSync(path.join(tmpDir, 'docs', '2026-03-02-error-monitor.md'), 'utf-8');
        expect(content).not.toContain('Previous report');
    });
    ...
    it('returns correct relative path', () => {
        const relPath = appendToDailyReport(tmpDir, 'perf-agent', runLog, '### Run\n\nTest.');
        expect(relPath).toBe('docs/2026-03-02-perf-agent.md');
    });
});
