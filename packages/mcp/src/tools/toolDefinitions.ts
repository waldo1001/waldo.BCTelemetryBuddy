...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
    ...
    },
    {
        ...
        description: 'Execute a KQL query against Business Central telemetry data. BEFORE writing your KQL, make sure you have informed yourself about the available fields and their data types by calling get_event_field_samples(eventId) — your KQL should be shaped by what the discovery tools told you, not by guessing. Prerequisites you MUST follow: (1) call get_event_catalog() to discover event IDs — without this you will not know which events exist, (2) call get_event_field_samples(eventId) for EVERY event ID you will filter or project on — this gives you the full list of available fields, their exact data types, and sample values so you can form correct KQL the first time. Skipping step 2 is the #1 cause of broken queries and wasted tokens because field types (especially TIMESPAN duration fields like executionTime/totalTime/serverTime which use "hh:mm:ss.fffffff" format, NOT numbers) cannot be guessed correctly. (3) If filtering by customer, call get_tenant_mapping() to get tenant IDs. Do NOT substitute `take 1 | project customDimensions` for get_event_field_samples — that workaround is forbidden.',
        inputSchema: {
            type: 'object',
            properties: {
