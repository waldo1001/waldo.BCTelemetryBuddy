...
import { startSdkStdioServer } from './mcpSdkServer.js';
        ...
        try {
            ...
            switch (rpcRequest.method) {
                case 'tools/list':
                    ...
                    // Uses single source of truth from toolDefinitions.ts
                    result = {
...
export async function startServer(config?: MCPConfig, mode?: 'stdio' | 'http'): Promise<void> {
    ...
    if (isStdioMode) {
        // Use the official MCP SDK for stdio mode (protocol 2025-06-18)
        await startSdkStdioServer(config);
    } else {
        // Use Express/legacy server for HTTP mode (backward compat with VS Code extension MCPClient)
