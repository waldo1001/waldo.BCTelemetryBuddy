...
### 9a. Keep knowledge-base/index.json up to date

**RULE: Any time you add or modify a file under `knowledge-base/`, you MUST regenerate `knowledge-base/index.json`.**

```bash
node scripts/generate-kb-index.js
# or via npm:
npm run generate-kb-index
```

- The generator reads YAML frontmatter from every `.md` file in `knowledge-base/` and rebuilds the index.
- The index is committed alongside the knowledge-base articles — it is **not** auto-generated by CI.
- CI runs `node scripts/generate-kb-index.js --check` and **fails the build** if `index.json` is stale.
- If you forget and CI fails with "index.json is stale", just run the command above and commit the result.
